...
Version **1.5.3** applies the workshop theme throughout the game: Home, Rooms, Upgrades, Treasures, gameplay, rewards, Endless, and popups. Every paper screen shares the same cream background, red top edge, square corners, and ink shadow. Buttons, disabled items, icons, instructions, and in-room dock/exit labels use matching styles. Each location keeps its distinct floor and furniture artwork. Opening Upgrades during a job asks once: **Keep playing** or **Quit & upgrade**, with the loss of unfinished cleaning and coins explained before quitting. Help and Settings opened from Pause return there; Main menu keeps the unfinished job paused.
