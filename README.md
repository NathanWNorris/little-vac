...
Version **1.5.4** simplifies the play screen to one bar: **clean percentage, bag space, and Pause**. The room gets more space, with a short control reminder and room/area number underneath. Pause contains Rooms, Upgrades, and optional job details with saved coins, carried dirt value, pending earnings, and medal times. Contextual dock, exit, and last-dirt guidance appears when needed. Every screen keeps the cream/red workshop theme and each location its distinct artwork. Opening Upgrades during a job asks once: **Keep playing** or **Quit & upgrade**. Help and Settings opened from Pause return there; Main menu keeps the unfinished job paused.
