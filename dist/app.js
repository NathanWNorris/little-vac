...
function gameHUD(){
  ...
  $('#bagStatus').textContent=r.unloading?'Emptying…':r.full?'Full → dock':fill>=.8?'Dock soon':'Room to spare';
  $('#bagValue').textContent=`In bag: ${money(b.bagValue)} ${b.bagValue===1?'coin':'coins'}`;
  $('#bagMeter').classList.toggle('full',fill>=.8);
  $('#coinText').textContent=money(career.coins);
  $('#pendingCoins').textContent=`+${money(r.coins+b.bagValue)} this ${areaCount()>1?'job':'room'}`;
  $('#pendingCoins').setAttribute('title',`${money(r.coins)} dropped off + ${money(b.bagValue)} in your bag. Saved when every area is finished.`);
  $('#elapsed').textContent=timeText(r.time);
  let tip=r.room.tip;
  if(!r.started)tip='Parked at drop-off. Left-click the room or press WASD to start.';
  else if(r.phase==='finishing')tip='All clean! Saving this job’s coins…';
  else if(r.unloading)tip='Emptying your bag. Stay beside the drop-off dock.';
  else if(r.full)tip='Bag full. Follow the green arrow to the drop-off dock. Your dirt stays in your bag between areas.';
  else if(fill>=.8)tip='Bag nearly full. Follow the green arrow to the drop-off dock to empty it.';
  else if(r.phase==='exiting')tip=`Area clear! Walk into the NEXT AREA door → ${r.room.exit.nextName}. Your bag comes with you.`;
  else if(lastFew)tip=`${remaining} ${remaining===1?'piece':'pieces'} left. Look for the bright yellow circles and pointers.`;
  else if(r.percent>=.95)tip=`${remaining} pieces left. Golden rings mark every remaining bit, including faint dust.`;
  else if(r.percent>=.8)tip='Almost done. Golden rings mark the remaining mess.';
  else if(r.room.id===1&&r.collectedCount<8)tip=window.innerWidth<=660||window.matchMedia?.('(pointer: coarse)').matches?'Drag the pad or room to glide. Vacuuming is automatic.':'Move your mouse to steer — no holding needed. WASD / arrows work too.';
  else if(r.room.id===1&&r.time<30)tip='Dock to empty your bag. Reach 100% clean to finish, earn coins, and open the next room.';
  $('#gameTip').textContent=tip;
  if(areaCount()>1){
    $('#areaGuide').classList.toggle('exit-open',r.phase==='exiting'&&fill<.8);
    $('#areaGuide').classList.toggle('dock-needed',fill>=.8);
    $('#areaHint').textContent=r.unloading?'Emptying at the drop-off dock…':fill>=.8?'Follow the green arrow to empty your bag.':r.phase==='exiting'?'Area clear → Next door. Your bag comes with you.':r.room.toughness>1.4?'Heavy dust: linger over it. Stronger suction clears it faster.':r.room.exit?'Clean 100%, then follow the arrow to the next area.':'Final area. Clean 100% to finish this job.';
  }
}
