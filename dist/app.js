...
async function startRoom(id,seed){
  ...
  try{const unlocked=await save(c=>{if(!isRoomUnlocked(c,id))return false;c.lastRoom=id;return true;});if(!unlocked){notify('Finish the previous room first.');return;}
    ...
    run=createRun(room,statsFor(career.upgrades),`${Date.now()}-${crypto.randomUUID?.()||Math.random()}`);run.careerAtStart=career;run.started=false;settlement=null;roomDistrict=room.location;playRoom();
  }finally{starting=false;}
}
function readyToStart(){return screen==='play'&&run?.phase==='playing'&&!run.started;}
function startInstructions(){return `<div id="roomStart" class="room-start" ${run.started?'hidden':''} role="region" aria-labelledby="startTitle"><div class="start-card"><h2 id="startTitle">Ready to clean?</h2><div class="start-inputs" aria-label="Two ways to steer"><div><strong>Mouse</strong><span>Move the cursor</span></div><span class="input-or">OR</span><div><strong>W A S D</strong><span>Arrow keys work too</span></div></div><p class="start-steer"><span class="mouse-copy">No holding needed. Vacuuming is automatic.</span><span class="touch-copy"><strong>Drag the pad or room to steer.</strong><br>Vacuuming is automatic.</span></p><p class="start-rules"><strong>Bag full?</strong> Park beside a green dock.<br><strong>95% clean?</strong> Room done. Spend your coins on upgrades.</p>${action('begin-room','<span class="mouse-copy">Left-click to start</span><span class="touch-copy">Tap to start</span>','primary')}<p class="start-keys">Enter to start · Esc to pause</p></div></div>`;}
...
function beginRoom(){
  ...
  $('#announcement').textContent='Room started. Guide the vacuum, dock when the bag is full, and clean 95% to finish.';
  return true;
}
function playRoom(){const room=run.room,id=room.id;prepareScreen('play');main.innerHTML=`<div class="play-head"><div><div class="eyebrow">${id===0?'ENDLESS SHIFT':`${esc(LOCATIONS[room.location].name)} / ROOM ${String(id).padStart(2,'0')}`}</div><h1>${esc(room.name)}</h1><p class="steering-guide"><span class="mouse-copy"><strong>Mouse</strong> or <strong>WASD</strong> to steer<small> · Arrow keys work too</small></span><span class="touch-copy"><strong>Drag</strong> the pad or room to steer</span></p></div><div class="play-tools">${action('pause','Ⅱ Pause')}</div></div><section class="game-hud"><div class="clean-readout"><strong id="cleanPercent">0%</strong><span>ROOM<br>CLEAN<small class="finish-target">Finish at 95%</small></span></div><div id="bagMeter" class="bag-meter"><div class="bag-label"><strong id="bagText">Bag 0 / ${run.stats.capacity}</strong><span id="bagStatus">Room to spare</span></div><div class="progress-track"><div class="progress-fill" id="bagFill" style="width:0%"></div></div></div><div class="coin-hud"><small>SAVED COINS</small><span id="coinText">${money(career.coins)}</span><small id="pendingCoins" title="Added to your saved coins when you finish the room">+0 this room</small></div></section><div class="arena"><canvas id="gameCanvas" width="960" height="640" tabindex="0" aria-label="Cleaning room. Left-click Start before cleaning. Then move your mouse without holding, or use WASD or arrow keys. Touch: tap Start, then drag to steer. Suction and emptying at green docks are automatic. Clean 95% to finish. Escape pauses."></canvas><div class="joystick" aria-hidden="true"><div class="knob"></div></div><div class="touch-hint">Drag to glide</div>${startInstructions()}</div><div class="game-bottom"><p id="gameTip">${esc(room.tip)}</p><details><summary>Room details</summary><div class="stat-details"><span id="elapsed">0:00</span><br>Gold ${timeText(room.goldTime)} · Silver ${timeText(room.silverTime)}<br>${id===0?`Seed: ${esc(room.seed)}<br>`:''}95% clean = finished</div></details></div>`;setupCanvas();bindActions();if(!modal.open)focusRoom();lastStamp=performance.now();accumulator=0;lastHud=0;gameHUD();}
...
function pointerDown(e){
  if(screen!=='play'||paused||modal.open||starting||run?.phase!=='playing')return;
  if(!run.started){
...
function pointerMove(e){if(screen!=='play'||paused||modal.open||!run?.started||run.phase!=='playing')return;if(remoteControl)clearInput();input.pointerMove(e);}
...
document.addEventListener('keydown',e=>{if(e.ctrlKey||e.metaKey||e.altKey)return;if(e.key==='Escape'){e.preventDefault();if(e.repeat)return;if(modal.open)closeModal();else pause();return;}if(screen!=='play'||modal.open||/INPUT|TEXTAREA|SELECT/.test(e.target.tagName))return;if(e.key===' '&&e.target.closest('button,a,summary'))return;if(!run?.started){if(['w','a','s','d','ArrowUp','ArrowDown','ArrowLeft','ArrowRight',' '].includes(e.key))e.preventDefault();return;}if(remoteControl)clearInput();const key=e.key.length===1?e.key.toLowerCase():e.key;if(['w','a','s','d','ArrowUp','ArrowDown','ArrowLeft','ArrowRight',' '].includes(key)){e.preventDefault();if(key!==' ')input.clear();keys.add(key);}});
...
function frame(stamp){if(remoteControl&&stamp>=remoteControl.until){const control=remoteControl;remoteControl=null;control.resolve({interrupted:false,...publicState()});}const delta=Math.min(.1,(stamp-(lastStamp||stamp))/1000);lastStamp=stamp;if(screen==='play'&&run&&ctx){if(run.started&&!paused&&!modal.open&&!starting&&!settling&&!resetting&&!document.hidden){accumulator+=delta;let input=movement();while(accumulator>=1/120){step(run,1/120,input);accumulator-=1/120;}for(const e of run.events){sound(e.type,e.material);if(e.type==='trinket')notify(`Found: ${run.trinket.name}!`);if(e.type==='unload')notify(`Bag emptied. ${e.value} ${e.value===1?'coin':'coins'} ready to bank when you finish.`);if(e.type==='full')$('#announcement').textContent='Bag full. Move to a collection station.';}run.events=[];if(run.phase==='complete'){completed();requestAnimationFrame(frame);return;}}
