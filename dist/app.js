...
function renderHome(focus=true){
  ...
  main.innerHTML=`<section class="home-menu"><canvas id="titleArt" class="title-art" width="960" height="640" aria-hidden="true"></canvas><div class="title-content"><h1 class="title-logo">SWEEP <span>SHIFT</span></h1><nav class="title-options" aria-label="Main menu">${action('continue',`<span>${label}</span><span class="title-play-arrow" aria-hidden="true">▶</span>`,'home-play title-choice')}${action('rooms','Rooms','title-choice')}${action('shop',`<span>Upgrades${active?' <small>Locked</small>':count?` <small>${count} ready</small>`:''}</span>`,'title-choice',active?'title="Finish or quit this job before upgrading"':'')}${action('controls','How to play','title-choice')}</nav><div class="title-job"><p>${esc(location)}</p><h2>${esc(heading)}</h2><p>${esc(detail)}</p>${active?'<p>Your room stays paused here.</p>':''}</div><p class="title-record"><span>${career.completed.length} / 24 rooms complete</span><span>${money(career.coins)} saved coins</span></p></div></section>`;
...
function startInstructions(){return `<div id="roomStart" class="room-start" ${run.started?'hidden':''} role="region" aria-labelledby="startTitle"><div class="start-card"><h2 id="startTitle">Ready to clean?</h2>${areaCount()>1?`<p class="start-area">Area ${areaNumber()} of ${areaCount()} · ${esc(run.room.areaName)}</p>`:''}${quickGuide()}${action('begin-room','<span class="mouse-copy">Left-click to start ▶</span><span class="touch-copy">Tap to start ▶</span>','primary')}<p class="start-keys">Or press WASD / arrow keys · Esc pauses</p></div></div>`;}
...
function gameHUD(){
  ...
  let tip=r.room.tip;
  if(!r.started)tip=window.matchMedia?.('(pointer: coarse)').matches?'Tap Start, then drag the pad or room to steer.':'Parked at drop-off. Left-click the room or press WASD to start.';
