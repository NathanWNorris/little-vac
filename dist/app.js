...
function areaGuide(){if(areaCount()===1)return '';return `<div id="areaGuide" class="area-guide"><div><strong id="areaHeading">Area ${areaNumber()} of ${areaCount()} · ${esc(run.room.areaName)}</strong><span id="areaHint">${run.room.exit?'Clean 100%, then follow the arrow to the next area.':'Final area. Clean 100% to finish this job.'}</span></div><ol class="area-steps" aria-label="Areas in this job">${Array.from({length:areaCount()},(_,i)=>`<li class="${i<areaNumber()-1?'done':i===areaNumber()-1?'current':''}" ${i===areaNumber()-1?'aria-current="step"':''} aria-label="Area ${i+1}${i<areaNumber()-1?', cleaned':''}">${i<areaNumber()-1?'✓':i+1}</li>`).join('')}</ol></div>`;}
function startInstructions(){return `<div id="roomStart" class="room-start" ${run.started?'hidden':''} role="region" aria-labelledby="startTitle"><div class="start-card"><h2 id="startTitle">Parked at the dock</h2>${areaCount()>1?`<p class="start-area">Area ${areaNumber()} of ${areaCount()} · ${esc(run.room.areaName)}</p>`:''}<div class="start-inputs" aria-label="Two ways to steer"><div><strong>Mouse</strong><span>Left-click, then steer</span></div><span class="input-or">OR</span><div><strong>W A S D</strong><span>Press to start & move</span></div></div><p class="start-steer"><span class="mouse-copy">Left-click the room or press WASD to begin.</span><span class="touch-copy"><strong>Drag the pad or room to steer.</strong><br>Vacuuming is automatic.</span></p><p class="start-rules"><strong>Bag full?</strong> Park beside a green dock.<br><strong>100% clean?</strong> ${run.room.exit?'Follow the arrow into the next area.':'Room done. Spend your coins on upgrades.'}${areaCount()>1?'<br>Finish every area to save this job’s coins.':''}${run.room.toughness>1.4?'<br><strong>Heavy dust:</strong> linger over it. Stronger suction clears it faster.':''}</p>${action('begin-room','<span class="mouse-copy">Left-click to start</span><span class="touch-copy">Tap to start</span>','primary')}<p class="start-keys">Arrow keys also start · Esc to pause</p></div></div>`;}
...
function playRoom(){const room=run.room,id=room.id;prepareScreen('play');main.innerHTML=`<div class="play-head"><div><div class="eyebrow">${id===0?'ENDLESS SHIFT':`${esc(LOCATIONS[room.location].name)} / ROOM ${String(id).padStart(2,'0')}`}</div><h1>${esc(room.name)}</h1><p class="steering-guide"><span class="mouse-copy"><strong>Mouse</strong> or <strong>WASD</strong> to steer<small> · Arrow keys work too</small></span><span class="touch-copy"><strong>Drag</strong> the pad or room to steer</span></p></div><div class="play-tools">${action('pause','Ⅱ Pause')}</div></div>${areaGuide()}<section class="game-hud"><div class="clean-readout"><strong id="cleanPercent">0%</strong><span>${areaCount()>1?'AREA':'ROOM'}<br>CLEAN<small class="finish-target">${run.room.exit?'Opens at 100%':'Finish at 100%'}</small></span></div><div id="bagMeter" class="bag-meter"><div class="bag-label"><strong id="bagText">Bag 0 / ${run.stats.capacity}</strong><span id="bagStatus">Room to spare</span></div><div class="progress-track"><div class="progress-fill" id="bagFill" style="width:0%"></div></div></div><div class="coin-hud"><small>SAVED COINS</small><span id="coinText">${money(career.coins)}</span><small id="pendingCoins" title="Saved after every area in this job is complete">+0 this room</small></div></section><div class="arena"><canvas id="gameCanvas" width="960" height="640" tabindex="0" aria-label="Cleaning room. Parked at the drop-off dock. Left-click inside the room, or press WASD or an arrow key to start and steer. After clicking, move your mouse without holding. Touch: tap Start, then drag to steer. Suction and emptying at green docks are automatic. Clean 100% in each area. If a next-area door opens, follow its arrow and walk into it. Finish all areas to save your coins. Escape pauses."></canvas><div class="joystick" aria-hidden="true"><div class="knob"></div></div><div class="touch-hint">Drag to glide</div>${startInstructions()}</div><div class="game-bottom"><p id="gameTip">${esc(room.tip)}</p><details><summary>Room details</summary><div class="stat-details"><span id="elapsed">0:00</span><br>Gold ${timeText(room.goldTime)} · Silver ${timeText(room.silverTime)}<br>${id===0?`Seed: ${esc(room.seed)}<br>`:''}${areaCount()>1?'100% per area · Finish every area to save coins':'100% clean = finished'}${room.toughness>1.4?'<br>Heavy dust needs steady suction. Upgrades make it faster.':''}</div></details></div>`;setupCanvas();bindActions();if(!modal.open)focusRoom();lastStamp=performance.now();accumulator=0;lastHud=0;gameHUD();}
...
function pointerDown(e){
  ...
  if(!run.started){
    ...
    e.preventDefault?.();
    if(!beginRoom())return;
    // Starting does not steer toward the click. A touch may continue into a drag.
...
function gameHUD(){if(!run||screen!=='play')return;const r=run,b=r.robot,pct=Math.min(100,Math.floor(r.percent*100)),fill=b.bag/r.stats.capacity;$('#cleanPercent').textContent=pct+'%';$('#bagText').textContent=`Bag ${b.bag} / ${r.stats.capacity}`;$('#bagFill').style.width=(fill*100)+'%';$('#bagStatus').textContent=r.unloading?'Emptying…':r.full?'Full → dock':fill>=.8?'Dock soon':'Room to spare';$('#bagMeter').classList.toggle('full',fill>=.8);$('#coinText').textContent=money(career.coins);$('#pendingCoins').textContent=`+${money(r.coins+b.bagValue)} this ${areaCount()>1?'job':'room'}`;$('#elapsed').textContent=timeText(r.time);let tip=r.room.tip;if(!r.started)tip='Parked at drop-off. Left-click the room or press WASD to start.';else if(r.phase==='exiting')tip=`Area clear! Follow the arrow and walk into the NEXT AREA door → ${r.room.exit.nextName}.`;else if(r.phase==='finishing')tip='All clean! Saving this job’s coins…';else if(r.unloading)tip='Emptying your bag. Stay beside the dock.';else if(r.full)tip='Bag full. Follow the dock marker — your robot still moves normally.';else if(fill>=.8)tip='Nearly full. Park beside a collection station to empty automatically.';else if(r.percent>=.95){const left=r.debris.filter(d=>!d.collected).length;tip=`${left} ${left===1?'piece':'pieces'} left. Golden rings mark every remaining bit, including faint dust.`;}else if(r.percent>=.8)tip='Almost done. Golden rings mark the remaining mess.';else if(r.room.id===1&&r.collectedCount<8)tip=window.innerWidth<=660||window.matchMedia?.('(pointer: coarse)').matches?'Drag the pad or room to glide. Vacuuming is automatic.':'Move your mouse to steer — no holding needed. WASD / arrows work too.';else if(r.room.id===1&&r.time<30)tip='Dock to empty your bag. Reach 100% clean to finish, earn coins, and open the next room.';$('#gameTip').textContent=tip;if(areaCount()>1){$('#areaGuide').classList.toggle('exit-open',r.phase==='exiting');$('#areaHint').textContent=r.phase==='exiting'?`Area clear → Walk into the NEXT AREA door.`:r.room.toughness>1.4?'Heavy dust: linger over it. Stronger suction clears it faster.':r.room.exit?'Clean 100%, then follow the arrow to the next area.':'Final area. Clean 100% to finish this job.';}}
...
function controls(){if(modal.open)modal.close();openModal(`<h2 id="modalTitle">How to play</h2><div class="controls-help"><p><strong>Start from the dock</strong><br>Each new job starts parked in its drop-off dock. Left-click inside the room, or press WASD or an arrow key to start moving. On a touchscreen, tap Start. The clock and vacuum stay stopped while you wait.</p><p><strong>Steer with your mouse OR WASD</strong><br><strong>Mouse:</strong> move the cursor over the room. The vacuum follows without holding a button. Move the cursor outside the room to stop.<br><strong>Keyboard:</strong> use W, A, S, D or the arrow keys. Release the keys to stop.<br><strong>Touch:</strong> drag the pad or room. Vacuuming is automatic with every control. Press Escape to pause.</p><p><strong>Dock to empty</strong><br>Each area has one green dock. Park beside it to empty your bag automatically. A full bag stops vacuuming, but you can still move.</p><p><strong>Finish at 100%</strong><br>Collect every piece of dirt to clear an area. Your coins are saved when every area is finished. Golden rings reveal the remaining mess near the end. Time medals and treasures are optional.</p><p><strong>Follow the arrow in bigger rooms</strong><br>From room 9, jobs have two to four connected areas. Clean 100% in one area to open the NEXT AREA door, then walk into it. Walk through a right-hand door to enter the next area from the left. Keep steering; there is no new start screen. Finish every area to save the job’s coins. The treasure is in the final area.</p><p><strong>Heavy dust</strong><br>Later rooms have denser dust and stuck leaves. Linger over them to clear them. Stronger suction makes this faster; every room can still be finished without it.</p><p><strong>Upgrade between rooms</strong><br>Finish the room, or pause and quit it. Quitting discards this room’s unfinished cleaning and coins.</p></div><div class="button-row">${action('resume',screen==='play'?'Resume':'Got it','primary')}</div>`);}
  ...
  if(screen==='play'&&run&&ctx){
    if(run.started&&!paused&&!modal.open&&!starting&&!settling&&!resetting&&!document.hidden){
      ...
      for(const e of run.events){
        ...
        if(e.type==='area-clear'){$('#announcement').textContent='Area clear! Follow the arrow and walk into the next-area door.';sound('unload');}
      ...
      if(entered){
        ...
        $('.finish-target').textContent=run.room.exit?'Opens at 100%':'Finish at 100%';
        gameHUD();
        $('#announcement').textContent=`Entered area ${areaNumber()} of ${areaCount()}: ${run.room.areaName}. Keep steering with your mouse or WASD. Clean every piece to continue.`;
      }
