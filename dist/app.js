...
import {esc,timeText,money,coinBalance,menuHeading,action,affordable,navigation,pausedRoomBanner,roomsMarkup,shopMarkup,guideArt} from './ui.js';
...
function careerChanged(next){
  ...
  const lockedMenu=(screen==='endless'||screen==='ending')&&next.completed.length<24;
  if(lockedMenu||(run&&(careerWasReset(run.careerAtStart,next)||!isRoomUnlocked(next,run.room.id)))){
...
function updateNav(){main.querySelectorAll('.menu-wallet').forEach(wallet=>{wallet.outerHTML=coinBalance(career.coins,'menu-wallet');});const nav=$('#gameNav');if(!nav)return;nav.innerHTML=navigation(career,screen,hasActiveRun());bindActions(nav,false);}
function settingsApply(){
  document.body.classList.toggle('no-motion',career.settings.reducedMotion);$('#soundBtn').textContent=career.settings.muted?'♫̸':'♪';$('#soundBtn').setAttribute('aria-label',career.settings.muted?'Unmute sound':'Mute sound');$('#soundBtn').setAttribute('aria-pressed',String(!career.settings.muted));
  if(modal.open){
    const sound=content.querySelector('#muteSetting'),volume=content.querySelector('#volumeSetting'),motion=content.querySelector('#motionSetting');
    if(sound)sound.checked=!career.settings.muted;
    if(volume)volume.value=String(career.settings.effects);
    if(motion)motion.checked=career.settings.reducedMotion;
  }
}
...
function bindActions(root=main,focusHeading=true){if(root===main&&screen!=='play'&&screen!=='title'&&screen!=='shop'&&hasActiveRun()&&!root.querySelector('.paused-room'))(root.querySelector('.menu-head')||root).insertAdjacentHTML(root.querySelector('.menu-head')?'afterend':'afterbegin',pausedRoomBanner(run));root.querySelectorAll('[data-action]').forEach(b=>b.addEventListener('click',()=>{Promise.resolve(act(b.dataset.action,b)).catch(error=>{console.error(error);notify('That action could not finish. Please try again.');});}));if(root===main&&screen!=='play'&&focusHeading){const heading=root.querySelector('h1');if(heading){heading.tabIndex=-1;heading.focus({preventScroll:true});}}}
...
function syncCanvasAccessibility(){
  canvas.tabIndex=run.started?0:-1;
  const controls=run.started?'Move your mouse without holding, or use WASD or arrow keys to steer. Touch: drag to steer.':'Parked at the drop-off dock. Left-click inside the room, or press WASD or an arrow key to start and steer. After clicking, move your mouse without holding. Touch: tap Start, then drag to steer.';
  canvas.setAttribute('aria-label',`Cleaning room. ${controls} Suction and emptying at green docks are automatic. Clean 100% in each area. Clean this area to unlock its hallway. Walk through open hallways in either direction. Finish all areas to save your coins. Escape pauses.`);
}
function setupCanvas(){canvas=$('#gameCanvas');syncCanvasAccessibility();ctx=canvas.getContext('2d');const dpr=Math.min(2,window.devicePixelRatio||1);canvas.width=960*dpr;canvas.height=640*dpr;ctx.setTransform(dpr,0,0,dpr,0,0);const surface=$('.arena');surface.addEventListener('pointerdown',pointerDown);surface.addEventListener('pointermove',pointerMove);surface.addEventListener('pointerup',pointerUp);surface.addEventListener('pointercancel',e=>input.pointerCancel(e));surface.addEventListener('lostpointercapture',e=>input.pointerCancel(e));canvas.addEventListener('pointerleave',e=>input.pointerLeave(e));}
...
function beginRoom(){
  ...
  clearInput();run.started=true;syncCanvasAccessibility();lastStamp=performance.now();accumulator=0;
...
function playRoom(){
  ...
  main.innerHTML=`<h1 class="sr-only">${esc(room.name)}</h1>
    ...
    <div class="arena"><canvas id="gameCanvas" width="960" height="640" tabindex="-1" aria-label="Cleaning room"></canvas><div class="joystick" aria-hidden="true"><div class="knob"></div></div><div class="touch-hint">Drag to glide</div>${startInstructions()}</div>
...
function collection(){prepareScreen('collection');main.innerHTML=`${menuHeading('Treasures',career)}<p class="menu-hint">${career.trinkets.length} / 24 found. One per job, in its final area.</p><div class="button-row">${action('rooms','Back to rooms','primary')}</div>${LOCATIONS.map((l,i)=>`<div class="section-head"><h2>${esc(l.name)}</h2></div><div class="shelf">${l.trinkets.map((t,j)=>{let found=career.trinkets.includes(i*6+j+1);return`<div class="trinket-card ${found?'':'missing'}"><span>${found?'✧':'?'}</span><small>${found?esc(t):`Hidden in room ${i*6+j+1}`}</small></div>`}).join('')}</div>`).join('')}`;bindActions();}
function newSeed(){return `${['sunny','mint','cosy','little','bright','coral'][Math.floor(Math.random()*6)]}-${Math.random().toString(36).slice(2,7)}`;}
function endless(){if(career.unlocked<25)return;prepareScreen('endless');main.innerHTML=`${menuHeading('Endless rooms',career)}<section class="result"><div class="result-icon">∞</div><div class="eyebrow" style="justify-content:center">ENDLESS SHIFT</div><p>Enter a seed to generate a room. Use the same seed to play it again.</p><form class="seed-form" id="seedForm"><input id="seedInput" aria-label="Room seed" value="${esc(newSeed())}" maxlength="40" required><button class="primary" type="submit">Play</button></form><p style="font-size:12px">Your purchased upgrades carry over.</p><div class="button-row">${action('shop','Robot upgrades')}${action('rooms','Back to rooms','ghost')}</div></section>`;$('#seedForm').onsubmit=e=>{e.preventDefault();let seed=$('#seedInput').value.trim().slice(0,40);if(seed)requestStart(0,seed);};bindActions();}
async function ending(){if(career.completed.length<24)return;const revision=viewRevision;const eligible=await save(c=>{if(c.completed.length<24)return false;c.endingSeen=true;return true;});if(revision!==viewRevision)return;if(!eligible||career.completed.length<24)return rooms();prepareScreen('ending');main.innerHTML=`${menuHeading('All rooms complete!',career)}<section class="result"><div class="eyebrow" style="justify-content:center">CAMPAIGN COMPLETE</div><div class="ending-stamp" aria-label="24 of 24 rooms cleaned"><strong>24 <span>/ 24</span></strong><span>ROOMS CLEANED</span></div><p>You cleaned all 24 rooms across four locations.</p><p>Replay rooms for better medals, find missing treasures, or try Endless.</p><div class="bonus-note">Endless Shift and your final robot shell are unlocked.</div><div class="button-row">${action('endless','Play endless','primary')}${action('collection','View treasures')}${action('credits','Credits','ghost')}</div><div class="button-row">${action('rooms','Choose a room','ghost')}</div></section>`;bindActions();}
function openModal(html,kind='',initialFocus='action'){
  lastFocus=document.activeElement;paused=screen==='play';clearInput();dialogScreen=kind;content.innerHTML=html;bindActions(content);modal.showModal();
  if(initialFocus==='heading'){
    const heading=content.querySelector('#modalTitle');
